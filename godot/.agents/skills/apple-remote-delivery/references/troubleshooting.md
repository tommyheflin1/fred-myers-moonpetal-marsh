...
| Automatic archive says team has no devices | Do not invent or register a device. If the established app release used an installed App Store distribution profile, set only that app's `signing_style: manual` and exact `app_store_profile_uuid`. The same lane validates app/team, expiry, distribution type, native capability and matching installed Apple Distribution identity. Preserve previous checkpoints and use a clean new checkout if source configuration changes. Never reuse another app's profile. |
