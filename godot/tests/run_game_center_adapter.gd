...
func _run() -> void:
	...
	var resumed_plugin := FakeGameCenter.new()
	resumed_plugin.authenticated = true
	check(adapter.configure(resumed_plugin), "cold adapter accepts an already authenticated device")
	check(adapter.begin_sign_in() and adapter.state == "authenticating", "already authenticated device refreshes provider metadata before requesting proof")
	check(resumed_plugin.identity_signature_requests == 0, "cold adapter cannot reuse an empty or stale account context")
	resumed_plugin.events.append({"type": "authentication", "result": "ok", "displayName": "Fictional Returning Frog", "team_player_id": "fictional-returning-team", "game_player_id": "fictional-returning-game"})
	adapter.poll()
	check(adapter.state == "awaiting_signature" and resumed_plugin.identity_signature_requests == 1, "refreshed provider metadata requests a new proof")
	check(adapter.display_name == "Fictional Returning Frog" and adapter.team_player_id == "fictional-returning-team", "cold sign-in uses the current provider name and account")
	var identity_results: Array[Dictionary] = []
	adapter.sign_in_completed.connect(func(result: Dictionary) -> void: identity_results.append(result.duplicate(true)))
	resumed_plugin.events.append({"type": "identity_verification_signature", "result": "ok", "team_player_id": "fictional-switched-team", "game_player_id": "fictional-switched-game", "public_key_url": "https://static.gc.apple.com/public-key", "signature": "fictional-switched-proof", "salt": "fictional-salt", "timestamp": int(Time.get_unix_time_from_system() * 1000.0)})
	adapter.poll()
	check(identity_results.size() == 1 and not bool(identity_results[0].verified_signature), "account switch during signature callback cannot attach the previous account's name")
	check(adapter.begin_sign_in(), "a new explicit name review refreshes an already authenticated adapter")
	check(adapter.display_name.is_empty() and adapter.team_player_id.is_empty(), "reconnect never exposes stale identity while provider metadata is pending")
	resumed_plugin.events.append({"type": "authentication", "result": "ok", "displayName": "Fictional Renamed Frog", "team_player_id": "fictional-returning-team", "game_player_id": "fictional-returning-game"})
	resumed_plugin.events.append({"type": "identity_verification_signature", "result": "ok", "team_player_id": "fictional-returning-team", "game_player_id": "fictional-returning-game", "public_key_url": "https://static.gc.apple.com/public-key", "signature": "fictional-refreshed-proof", "salt": "fictional-salt", "timestamp": int(Time.get_unix_time_from_system() * 1000.0)})
	adapter.poll()
	check(identity_results.size() == 2 and bool(identity_results[1].verified_signature) and identity_results[1].display_name == "Fictional Renamed Frog", "fresh review pairs the current provider name with a matching new account proof")

	for index in range(100):
		resumed_plugin.events.append({"type": "fictional_noise"})
	adapter.poll()
	check(resumed_plugin.events.size() == 68, "callback flood yields after 32 events so UI and timeout timers run")
	adapter.poll()
	adapter.poll()
	adapter.poll()
	check(resumed_plugin.events.is_empty(), "bounded callback processing eventually drains without dropping events")
	check(adapter.begin_sign_in(), "authentication can restart after callback flood")
	resumed_plugin.events.append({"type": "authentication", "result": "ok", "displayName": "Fictional Missing ID"})
	adapter.poll()
	check(adapter.state == "ready" and adapter.display_name.is_empty(), "successful callback without player identity fails closed and stays retryable")
