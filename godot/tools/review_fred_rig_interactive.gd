...
const SAVE_PREFIX := "user://m2_fred_rig_interactive_review"
const BOARD_PATH := "user://m2_fred_rig_interactive_review_board.json"
var game: Node2D
...
func _review() -> void:
	_clean()
	root.get_window().title = "Fred M2 Authored Rig Interactive Review"
	...
	game._set_feedback("[LOCAL RIG REVIEW] Isolated save; audio muted.")
