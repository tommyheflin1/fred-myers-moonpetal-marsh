...
expected_commit="${1:-}"
[[ -n "$expected_commit" ]] || { echo "Usage: tools/run_fred_app_build_3_macos.sh <exact-commit>" >&2; exit 2; }
[[ "${FRED_TESTFLIGHT_UPLOAD_ACK:-}" == "UPLOAD_BUILD_3" ]] || {
  echo "Set FRED_TESTFLIGHT_UPLOAD_ACK=UPLOAD_BUILD_3 only for the owner-approved TestFlight Build 3 upload." >&2
  exit 2
}
if [[ "$(uname -s)" == "Darwin" && "${FRED_IOS_CAFFEINATED:-0}" != "1" ]]; then
  command -v caffeinate >/dev/null || { echo "caffeinate is required." >&2; exit 1; }
  exec caffeinate -dimsu env FRED_IOS_CAFFEINATED=1 FRED_TESTFLIGHT_UPLOAD_ACK=UPLOAD_BUILD_3 \
...
archive="$PWD/builds/ios/FredMyers-AppBuild3.xcarchive"
export_dir="$PWD/builds/ios/AppBuild3-upload"
...
[[ "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' "$info_plist")" == "3" ]]

entitlements="$PWD/builds/ios/AppBuild3-entitlements.plist"
codesign -d --entitlements :- "$app" > "$entitlements" 2>/dev/null
/usr/libexec/PlistBuddy -c 'Print :com.apple.developer.game-center' "$entitlements" 2>/dev/null | grep -q true

options="$PWD/builds/ios/AppBuild3-ExportOptions.plist"
cat > "$options" <<EOF
...
upload_log="$export_dir/AppBuild3-upload.log"
xcrun altool --upload-app -f "$ipa" -t ios --apiKey "$api_key_id" \
  --apiIssuer "$api_issuer_id" --output-format json 2>&1 | tee "$upload_log"

echo "FRED_APP_BUILD_3_UPLOAD_SUCCEEDED commit=$expected_commit bundle=com.flinsvault.fredmyers version=1.0 build=3 ipa=$ipa ipa_sha256=$ipa_sha256"
