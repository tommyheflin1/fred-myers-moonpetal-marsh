...
check("Get-FredSha256" in launcher and "SHA256]::Create" in launcher, "launcher must use module-independent SHA-256")
check("manifest.files" in launcher, "launcher must verify pinned candidate files")
...
check("Get-FileHash" not in launcher, "Explorer launch must not depend on PowerShell module auto-loading")
...
check("safe.directory=$projectRoot" in installer, "installer must avoid a global Git safe-directory mutation")
check("git.Source @gitRepoArgs ls-files" in installer, "installer must inventory tracked candidate files")
