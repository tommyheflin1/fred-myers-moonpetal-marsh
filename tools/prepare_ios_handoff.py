#!/usr/bin/env python3
"""Create a clean, hash-guarded Fred App Build 1 macOS handoff bundle."""
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "builds/ios-handoff")
    ...
    bundle = output / "fred-myers-app-build-1.bundle"
    wrapper = output / "RUN-FRED-APP-BUILD-1.command"
...
git clone "$ROOT/fred-myers-app-build-1.bundle" "$SOURCE"
cd "$SOURCE"
git checkout --detach "$EXPECTED"
[[ "$(git rev-parse HEAD)" == "$EXPECTED" ]]
[[ -z "$(git status --porcelain)" ]]
export FRED_TESTFLIGHT_UPLOAD_ACK=UPLOAD_BUILD_1
bash tools/run_fred_app_build_1_macos.sh "$EXPECTED" | tee "$ROOT/FRED-APP-BUILD-1-RESULT.txt"
""" % expected,
        encoding="utf-8",
        newline="\n",
    )
